...
public class GitLogGenericPostPublishPerformer {
	...
	public GitLogGenericPostPublishPerformer(final ChangelogProcessor changelogProcessor, final String outputfile, final String fromRev, final String toRev) {
	...
	public boolean perform(final AbstractBuild<?, ?> build, final BuildListener listener) throws InterruptedException, IOException {
		...
		final GitChangelogArgs gitChangelogArgs = createGitChangelogArgs(build.getWorkspace(), resolveAndExpandIfNotNull(fromRev, env), resolveAndExpandIfNotNull(toRev, env));
		final PrintStream printStream = decideForPrintStream(listener);
		final Callable<String, IOException> task = new GitChangelogMasterToSlaveCallable(gitChangelogArgs);
	...
	private GitChangelogArgs createGitChangelogArgs(final FilePath workspace, final String fromRev, final String toRev) throws AbortException {
		if (workspace == null) {
